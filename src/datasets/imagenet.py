...
class ImageNet(ImageFolder):
    ...
    def __init__(self, root: str, split: str = 'train', download: bool = True, **kwargs: Any) -> None:
        ...
        if download is True:
            ...
            if not self.check_files_in_folder():
                raise FileNotFoundError('Tar files for imagenet dataset not found! Please set the download option to true.')
    ...
    def parse_archives(self) -> None:
        ...
        print('Archives parsed correctly...')
    ...
    def check_files_in_folder(self) -> bool:
        print('Checking if files are available in folder: {}'.format(self.root))
        for _, file_tuple in ARCHIVE_META.items():
            file_path = os.path.join(self.root, file_tuple[0])
    ...
    def download_files(self):
        ...
        data_folder = os.path.join(this_file_path, '..', '..', self.root)
        print('data_folder: {}'.format(data_folder))
        os.makedirs(data_folder, exist_ok=True)
        commands = ['wget https://image-net.org/data/ILSVRC/2012/{}'.format(file_tuple[0]) for _, file_tuple in ARCHIVE_META.items()]
        procs = [subprocess.Popen(commands[j], shell=True, cwd=data_folder) for j in range(len(commands))]
        for p in procs:
            p.wait()
...
def parse_val_archive(
    ...
    Args:
        ...
        folder (str, optional): Optional name for validation images folder. Defaults to
            'val'
    """
    print('Parsing validation archive...')
