...
def main():
    ...
    for query in tqdm(range(start, args.max_query)):
        ...
        #     queryIndex, invalidIndex, Precision[query], Recall[query] = query_strategies.random_sampling(args, unlabeledloader, len(labeled_ind_train), model_B, knownclass)
        # elif args.query_strategy == "eoal":
        #     if query == 0:
        #         queryIndex, invalidIndex, Prec, Rec = query_strategies.eoal_sampling(args, unlabeledloader, len(labeled_ind_train), model_A, model_bc, knownclass, use_gpu, None,None, True, args.diversity)            
        #     else:
        #         queryIndex, invalidIndex, Prec, Rec = query_strategies.eoal_sampling(args, unlabeledloader, len(labeled_ind_train), model_A, model_bc, knownclass, use_gpu, cluster_centers, cluster_labels, False, args.diversity)    
    ...
    np.savez('new_fair_results/{}_kc_{}_{}_seed_{}.npz'.format(args.query_strategy, args.known_class, args.dataset, args.seed), all_acc=all_accuracies, all_precision=all_precisions, all_recall=all_recalls) 
