...
class LL(ALMethod):
    ...
    def run(self):
        ...
        selection_result = np.argsort(-scores)[:self.args.query_batch]
        return selection_result, scores

    def get_predloss(self):
        self.models['backbone'].eval()
        self.models['module'].eval()
        loss_scores = torch.tensor([]).to(self.args.gpu)
        ...
        with torch.no_grad():
            ...
            for data in unlabeled_loader:
                # inputs = data[0].to(self.args.gpu)
                inputs = data[1][0].to(self.args.gpu)
