...
def get_simclr_augmentation(args, image_size):
    # parameter for resizecrop
    resize_scale = (args.resize_factor, 1.0) # resize scaling factor
    if args.resize_fix: # if resize_fix is True, use same scale
        resize_scale = (args.resize_factor, args.resize_factor)
...
def get_shift_module(args, eval=False):
    ...
    if not eval and not ('sup' in args.mode):
        assert args.batch_size == int(128/K_shift)
