...
def eval_unlabeled_detection(args, models,unlabeled_loader, train_loader,label_i_loader,simclr_aug=None):
    #base_path = os.path.split(args.load_path)[0]  # checkpoint directory
    prefix = f'{args.ood_samples}'
    if args.resize_fix:
        prefix += f'_resize_fix_{args.resize_factor}'
    else:
        prefix += f'_resize_range_{args.resize_factor}'

    #prefix = os.path.join(base_path, f'feats_{prefix}')
    kwargs = {
        'simclr_aug': simclr_aug,
        'sample_num': args.ood_samples,  # 10
        'layers': ['simclr', 'shift']
    }

    print("----------------Get unlabeled data's semantic and distinctive feature-------------")
    feats_u_distinctive, labels, feats_u_semantic, index = get_features(args, args.dataset, models['distinctive'], models['semantic'], unlabeled_loader, prefix=prefix, **kwargs)  # (N, T, d)
    ...
    for i in range(len(label_i_loader)):
        feats_l_i_distinctive, label_l_i, feats_l_i_semantic, index_l_i = get_features(args, f'{args.dataset}_train', models['distinctive'], models['semantic'],label_i_loader[i], prefix=prefix,**kwargs)  # (M, T, d)
        feats_labeled_i_distinctive.append(feats_l_i_distinctive)

    print("------------------Get labeled data's semantic and distinctive feature-------------")
    feats_l_semantic, label_l, feats_l_semantic, index_l = get_features(args, f'{args.dataset}_train', models['distinctive'], models['semantic'], train_loader,prefix=prefix,**kwargs)  # (M, T, d)
    ...
    for i in range(len(feats_labeled_i_distinctive)):
        ood_score = 'CSI'
        args.axis = []
        for f in feats_labeled_i_distinctive[i]['simclr'].chunk(args.K_shift, dim=1):
            axis = f.mean(dim=1)
            args.axis.append(axis.to(args.gpu))
        #print('axis size: ' + ' '.join(map(lambda x: str(len(x)), args.axis)))

        f_sim = [f.mean(dim=1) for f in feats_labeled_i_distinctive[i]['simclr'].chunk(args.K_shift, dim=1)]  # list of (M, d)
        f_shi = [f.mean(dim=1) for f in feats_labeled_i_distinctive[i]['shift'].chunk(args.K_shift, dim=1)]  # list of (M, 4)
        ...
        if ood_score == 'simclr':
            args.weight_sim = [1, 0, 0, 0]
            args.weight_shi = [0, 0, 0, 0]
        elif ood_score == 'CSI':
        ...
        scores_u_i = get_scores_distinctive(args, feats_u_distinctive).numpy()
    ...
    print('CSI_last')
    print(f'weight_sim:\t' + '\t'.join(map('{:.4f}'.format, args.weight_sim)))
    print(f'weight_shi:\t' + '\t'.join(map('{:.4f}'.format, args.weight_shi)))

    ood_score = 'simclr'
    args.axis = []
    for f in feats_l_semantic['simclr'].chunk(args.K_shift, dim=1):
        axis = f.mean(dim=1)
        args.axis.append(normalize(axis, dim=1).to(args.gpu))
    #print('axis size: ' + ' '.join(map(lambda x: str(len(x)), args.axis)))

    f_sim = [f.mean(dim=1) for f in feats_l_semantic['simclr'].chunk(args.K_shift, dim=1)]  # list of (M, d)
    f_shi = [f.mean(dim=1) for f in feats_l_semantic['shift'].chunk(args.K_shift, dim=1)]  # list of (M, 4)
    ...
    max_semantic,labels_semantic = get_scores_semantic(args, feats_u_semantic, label_l)
    ...
    query_index = []
    subset_index = []
    semantic_score = (1 - np.exp(-args.k * (max_semantic_score - args.t))) / (1 + np.exp(-args.k * (max_semantic_score - args.t)))
    ...
    for i in range(len(unlabeled_i_distinctive_score)):
        ...
        #score_i[labels_semantic != args.target_list[i]] = -1
        ...
        for j in range(len(subset_index_i)):
            if subset_index_i[j] not in subset_index:
                ...
                cnt+=1
            if cnt >= int(args.n_query)/args.known_class or len(subset_index) >= args.n_query:
                break
...
def Select_selector(select_indices,select_label, score,score_semantic,score_distinctive,args):
    ...
    finally_selector, query_inside = torch.topk(score, len(score)) #int(args.query_batch)/args.n_class
...
def get_scores_distinctive(args, feats_dict):
    # convert to gpu tensor
    feats_sim = feats_dict['simclr'].to(args.gpu)#[1600, 40, 128]
    feats_shi = feats_dict['shift'].to(args.gpu)#[1600, 40, 4]
    N = feats_sim.size(0)

    # compute scores
    scores = []
    for f_sim1, f_shi in zip(feats_sim, feats_shi):#
        f_sim = [f.mean(dim=0, keepdim=True) for f in f_sim1.chunk(args.K_shift)]  # list of (1, d)
        f_shi = [f.mean(dim=0, keepdim=True) for f in f_shi.chunk(args.K_shift)]  # list of (1, 4)
        f_sim_norm = [(f.mean(dim=0, keepdim=True)).norm(dim=1) for f in f_sim1.chunk(args.K_shift)]  # list of (1, d)

        score = 0
        for shi in range(args.K_shift):
            P_norm = args.axis[shi].norm(dim=1)
            cos_score = ((f_sim[shi] * args.axis[shi]).sum(dim=1)) / (f_sim_norm[shi].clone().detach() * P_norm.clone().detach())
            value, indices_e = cos_score.sort(descending=True)  # 降序
            score += torch.tensor(value[0].item()) - torch.tensor(value[1].item())
            anchor_A = (args.axis[shi][indices_e[0].item()]).view(1, -1)
            anchor_B = (args.axis[shi][indices_e[1].item()]).view(1, -1)
...
def get_scores_semantic(args, feats_dict,labels):
    # convert to gpu tensor
    feats_sim = feats_dict['simclr'].to(args.gpu)#[1600, 40, 128]
    feats_shi = feats_dict['shift'].to(args.gpu)#[1600, 40, 4]
    N = feats_sim.size(0)
    ...
    for f_sim, f_shi in zip(feats_sim, feats_shi):
        f_sim = [normalize(f.mean(dim=0, keepdim=True), dim=1) for f in f_sim.chunk(args.K_shift)]  # list of (1, d)
        f_shi = [f.mean(dim=0, keepdim=True) for f in f_shi.chunk(args.K_shift)]  # list of (1, 4)

        max_simi = 0
        for shi in range(args.K_shift):
            value_sim, indices_sim = ((f_sim[shi] * args.axis[shi]).sum(dim=1)).sort(descending=True)
            max_simi += value_sim.max().item() * args.weight_sim[shi]
...
def get_features(args, data_name, model_distinctive, model_semantic,loader, interp=False, prefix='',
    ...
    if len(left) > 0:
        _feats_dict_distinctive,labels,_feats_dict_semantic,index = _get_features(args, model_distinctive,model_semantic, loader, interp, args.dataset == 'imagenet',simclr_aug, sample_num, layers=left)
...
def _get_features(args, model_distinctive,model_semantic, loader, interp=False, imagenet=False, simclr_aug=None,
    ...
    for i, (indices, (x, label)) in enumerate(loader):
        labels.extend(label)
        ...
        for seed in range(sample_num):
            ...
            if args.K_shift > 1:
                x_t = torch.cat([args.shift_trans(hflip(x), k) for k in range(args.K_shift)])
            else:
            ...
            for layer in layers:
                ...
                if imagenet is False:
                    feats_batch_distinctive[layer] += feats_distinctive.chunk(args.K_shift)
                    feats_batch_semantic[layer] += feats_semantic.chunk(args.K_shift)
                else:
...
def test_classifier(args, model, loader, steps, marginal=False, logger=None):
    ...
    for n, (images, labels,index) in enumerate(loader):
        ...
        if marginal:
            ...
            for i in range(4):
                ...
                outputs += outputs_aux['joint'][:, args.n_classes * i: args.n_classes * (i + 1)] / 4.
        else:
