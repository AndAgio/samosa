...
# P == args
def csi_train_epoch(args, epoch, model, criterion, optimizer, scheduler, loader, simclr_aug=None, linear=None, linear_optim=None):

    assert simclr_aug is not None
    assert args.sim_lambda == 1.0  # to avoid mistake
    ...
    for n, (index, (images, labels)) in enumerate(loader):
        ...
        images1 = torch.cat([args.shift_trans(images1, k) for k in range(args.K_shift)])
        images2 = torch.cat([args.shift_trans(images2, k) for k in range(args.K_shift)])
        shift_labels = torch.cat([torch.ones_like(labels) * k for k in range(args.K_shift)], 0)  # B -> 4B
        ...
        penul_2 = outputs_aux['penultimate'][args.K_shift * batch_size: (args.K_shift + 1) * batch_size]
