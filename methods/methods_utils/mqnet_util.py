...
def get_labeled_features(args, models, labeled_in_loader):
    ...
    knownclass = get_splits(args.dataset, args.seed, args.known_class)

    features_in = torch.tensor([]).to(args.gpu)
    for data in labeled_in_loader:
        # inputs = data[0].to(args.gpu)
        inputs = data[1][0].to(args.gpu)
...
def get_unlabeled_features_LL_with_U(args, models, delta_loader, unlabeled_loader):
    ...
    features_unlabeled = torch.tensor([]).to(args.gpu)
    pred_loss = torch.tensor([]).to(args.gpu)
    in_ood_masks = torch.tensor([]).type(torch.LongTensor).to(args.gpu)
    indices = torch.tensor([]).type(torch.LongTensor).to(args.gpu)

    knownclass = get_splits(args.dataset, args.seed, args.known_class)

    for data in delta_loader:
        # inputs = data[0].to(args.gpu)
        # labels = data[1].to(args.gpu)
        # index = data[2].to(args.gpu)
        inputs, labels = data[1][0].to(args.gpu), data[1][1].to(args.gpu)
        # labels = lab_conv(knownclass, labels).to(args.gpu)
        index = torch.tensor(data[0]).to(args.gpu)

        in_ood_mask = labels.le(args.known_class - 1).type(torch.LongTensor).to(args.gpu)
...
def get_CSI_score(args, features_in, features_unlabeled):
    # CSI Score
    sim_scores = torch.tensor([]).to(args.gpu)
    ...
    return sim_scores.type(torch.float32).to(args.gpu).reshape((-1, 1))
